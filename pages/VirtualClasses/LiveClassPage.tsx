...
const LiveClassPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-100 px-4 sm:px-6 lg:px-8 py-8 flex flex-col">
      <motion.header
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        className="mb-6 sm:mb-8"
      >
        <h1 className="text-2xl sm:text-3xl font-bold text-[#2CB3C2] flex items-center">
          <FaVideo className="mr-2" /> {t('classeVirtuelle')}
        </h1>
      </motion.header>

      <div className="flex-1 flex flex-col lg:flex-row lg:space-x-6 space-y-6 lg:space-y-0">
        {/* Zone vidéo */}
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className="flex-1 bg-[rgba(44,179,194,0.54)] p-4 sm:p-6 rounded-xl shadow-lg"
        >
          <div className="bg-gray-300 h-64 sm:h-96 rounded-lg mb-4 flex items-center justify-center">
        ...
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className="w-full lg:w-80 bg-white p-4 rounded-xl shadow-lg"
        >
          <h2 className="text-base sm:text-lg font-semibold text-[#2CB3C2] mb-4 flex items-center">
            <FaComment className="mr-2" /> {t('chat')}
          </h2>
          <div className="h-48 sm:h-64 overflow-y-auto mb-4">
